...
// we moved this before emberInit to pre-initialize the tokens
  ...
  while (true) {
   ...
   // GPIO_PinModeSet(BSP_BULBPWM_COLOR_CC0_PORT,BSP_BULBPWM_COLOR_CC0_PIN,gpioModePushPull,1);

    	/*if(count==0)
    	{
       halBulbPwmDriverSetPwmLevel(tempp,1);
       tempp++;
       while(temp>65535)
        i
    	else if(count==1)
       halBulbPwmDriverSetPwmLevel(tempp,2);
    	else if(count==2)
       halBulbPwmDriverSetPwmLevel(tempp,3);
       tempp++;
        if(tempp>65535){
        tempp=0;
        count++;
